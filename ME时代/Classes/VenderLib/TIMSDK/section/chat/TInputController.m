...
- (void)textView:(TTextView *)textView didSendMessage:(NSString *)text
{
    ...
    data.head = kMeUnNilStr(kCurrentUser.header_pic);//data.head = kMeUnNilStr(kCurrentUser.header_pic);//TUIKitResource(@"default_head");
    data.content = text;
    data.isSelf = YES;
...
- (void)faceView:(TFaceView *)faceView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.section == 0){
        ...
        data.head = kMeUnNilStr(kCurrentUser.header_pic);//data.head = TUIKitResource(@"default_head");
        data.path = face.path;
