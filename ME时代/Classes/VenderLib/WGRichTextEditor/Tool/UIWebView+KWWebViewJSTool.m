...
- (void )setInputEnabled:(BOOL)enable{
    if(enable){
        [self stringByEvaluatingJavaScriptFromString:@"RE.setInputEnabled();"];
    }else{
        [self stringByEvaluatingJavaScriptFromString:@"RE.setInputDisEnabled();"];
    }
}
...
- (void)setupContent:(NSString *)content{
    /*
     ...
     ---------------------
     原文：https://blog.csdn.net/lidongxuedecsdn/article/details/50497998
     */
    NSString *charactersToEscape = @"?!@#$^&%*+,:;='\"`<>()[]{}/\\| ";
    NSCharacterSet *allowedCharacters = [[NSCharacterSet characterSetWithCharactersInString:charactersToEscape] invertedSet];
    NSString *html = [content stringByAddingPercentEncodingWithAllowedCharacters:allowedCharacters];
...
//    url = [[NSString stringWithFormat:kHKWEBATYPE] stringByAppendingString:url];
