...
@interface MEJoinOrganizationVC ()<UITableViewDelegate,UITableViewDataSource,AMapSearchDelegate,CLLocationManagerDelegate,RefreshToolDelegate>
...
@property (nonatomic, strong) MEJoinOrganizationModel *requestModel;

@end

@implementation MEJoinOrganizationVC
...
- (void)viewDidLoad {
    ...
    self.requestModel = [[MEJoinOrganizationModel alloc] init];
    ...
    [weakSelf.pulldownMenuView showWithItems:items
                        ...
                        pulldownMenuViewType:MEPulldownListViewRow
                               tapIndexBlock:^(NSArray * _Nonnull indexs) {
                                   ...
                                   weakSelf.requestModel.county = area;
                                   
                                   if ([area isEqualToString:@"全市"]) {
                                       weakSelf.requestModel.county = @"";
                                   }
...
- (void)showServiceTypeWithIndex:(NSInteger)index {
    ...
    [weakSelf.serviceTypeArr enumerateObjectsUsingBlock:^(MECustomerClassifyListModel *model, NSUInteger idx, BOOL * _Nonnull stop) {
        [tempArray addObject:kMeUnNilStr(model.type_name)];
    }];
    NSArray *items = [tempArray mutableCopy];
    [weakSelf.pulldownMenuView showWithItems:items
                                  isMultiple:NO
                        ...
                        pulldownMenuViewType:MEPulldownListViewRow
                               tapIndexBlock:^(NSArray *indexs) {
                                   ...
                                   weakSelf.requestModel.services_type = [NSString stringWithFormat:@"%@",@(model.idField)];
    ...
    [weakSelf.pulldownMenuView showWithItems:items
                        ...
                        pulldownMenuViewType:MEPulldownListViewRow
                               tapIndexBlock:^(NSArray *indexs) {
                                   ...
                                   if ([screenType isEqualToString:@"智能筛选"]) {
                                       weakSelf.requestModel.attention_num = @"";
                                       weakSelf.requestModel.volunteer_num = @"";
                                   }else if ([screenType isEqualToString:@"时数最多"]) {
                                       weakSelf.requestModel.attention_num = @"1";
                                   }else if ([screenType isEqualToString:@"人数最多"]) {
                                       weakSelf.requestModel.volunteer_num = @"1";
                                   }
...
- (ZLRefreshTool *)refresh{
    if(!_refresh){
        _refresh = [[ZLRefreshTool alloc]initWithContentView:self.tableView url:kGetApiWithUrl(MEIPcommonVolunteerOrganizationGetList)];
...
- (NSMutableArray *)screenTypeArr {
    if (!_screenTypeArr) {
        _screenTypeArr = @[@"智能筛选",@"时数最多",@"人数最多"].mutableCopy;
    }
