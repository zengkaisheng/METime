...
@interface MEStatisticsVC ()<JXCategoryViewDelegate,UIScrollViewDelegate>{
    NSArray *_arrType;
}
...
- (MEDiagnoseFeedBackBaseVC *)consultVC {
    if (!_consultVC) {
        ...
        _consultVC.view.frame = CGRectMake(0,0, SCREEN_WIDTH, SCREEN_HEIGHT-kMeNavBarHeight-kCategoryViewHeight-kMeTabBarHeight);
...
- (MEDiagnoseFeedBackBaseVC *)reoprtVC {
    if (!_reoprtVC) {
        ...
        _reoprtVC.view.frame = CGRectMake(SCREEN_WIDTH,0, SCREEN_WIDTH, SCREEN_HEIGHT-kMeNavBarHeight-kCategoryViewHeight-kMeTabBarHeight);
