...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    switch (component) {
        case 0: { // 年
            ...
            if (!self.isFutureTime) {
                
                if (year_integerValue.integerValue > time_integerValue) {
                    [pickerView selectRow:[self.dataArray[component] indexOfObject:self.timeArr[component]] inComponent:component animated:YES];
                    self.year = [NSString stringWithFormat:@"%ld年",time_integerValue];
        ...
        case 1: { // 月
            ...
            if (!self.isFutureTime) {
                ...
                if ([self.year integerValue] < [self.timeArr[0] integerValue]) {
                    ...
                    // 如果选择的月份大于当前月份 就刷新到当前月份
                    if (month_value.integerValue > [self.timeArr[component] integerValue]) {
                        [pickerView selectRow:[self.dataArray[component] indexOfObject:[NSString stringWithFormat:@"%ld月", [self.timeArr[component] integerValue]]] inComponent:component animated:YES];
                        self.month = [NSString stringWithFormat:@"%ld月",time_integerValue];
                        // 如果选择的月份小于当前月份，就直接赋值月份
                    } else {
                        self.month = month_value;
                        /// 刷新日
                        [self refreshDay];
        ...
        case 2: { // 日
            ...
            if (!self.isFutureTime) {
                ...
                self.day = [NSString stringWithFormat:@"%ld日",time_integerValue];
                //如果年份小于当前年份 计算日期之后直接赋值
                if ([self.year integerValue] < [self.timeArr[0] integerValue]) {
                    if (self.dayArr.count <= [dayStr integerValue]) {
                        self.day = day_value;
                    } else {
...
- (void)setIsShowHour:(BOOL)isShowHour {
    _isShowHour = isShowHour;
}
...
- (void)setMinimumDate:(NSString *)minimumDate {
    ...
    if (minimumDate.length > 0) {
        ...
        if (self.isShowHour) {
            ...
            [dateArr addObject:@"01时"];
            [dateArr addObject:@"01分"];
            self.timeArr = dateArr.mutableCopy;
        }
    }
}
